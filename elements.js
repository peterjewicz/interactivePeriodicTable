//All info here


var elements = {}

elements[1] = {};
elements[1]['number'] = 1;
elements[1]['symbol'] = "H";
elements[1]['name'] = "Hydrogen";
elements[1]['weight'] = 1.008;
elements[1]['meltingPoint'] = "13.99K/-259.16C/-434.49F";
elements[1]['boilingPoint'] = "20.271K/-252.879C/-423.182F";
elements[1]['year'] = 1766;
elements[1]['discoveredBy'] = 'Henry Cavendis';
elements[1]['isotopes'] = {}
elements[1]['isotopes'][0] = {}
elements[1]['isotopes'][1] = {}
elements[1]['isotopes'][0]['name'] = "Hydrogen";
elements[1]['isotopes'][0]['na'] = '99.9885%';
elements[1]['isotopes'][0]['halfLife'] = 'stable'
elements[1]['isotopes'][1]['name'] = "Deuterium";
elements[1]['isotopes'][1]['na'] = '.0115%';
elements[1]['isotopes'][1]['halfLife'] = 'stable'



elements[2] = {};
elements[2]['number'] = 2;
elements[2]['symbol'] = "He";
elements[2]['name'] = "Helium";
elements[2]['weight'] = 4.002;

elements[3] = {};
elements[3]['number'] = 3;
elements[3]['symbol'] = "Li";
elements[3]['name'] = "Lithium";
elements[3]['weight'] = 6.94;

elements[4] = {};
elements[4]['number'] = 4;
elements[4]['symbol'] = "Be";
elements[4]['name'] = "Beryllium";
elements[4]['weight'] = 9.012;

elements[5] = {};
elements[5]['number'] = 5;
elements[5]['symbol'] = "B";
elements[5]['name'] = "Boron";
elements[5]['weight'] = 10.811;

elements[6] = {};
elements[6]['number'] = 6;
elements[6]['symbol'] = "C";
elements[6]['name'] = "Carbon";
elements[6]['weight'] = 12.01;

elements[7] = {};
elements[7]['number'] = 7;
elements[7]['symbol'] = "N";
elements[7]['name'] = "Nitrogen";
elements[7]['weight'] = 14.006;

elements[8] = {};
elements[8]['number'] = 8;
elements[8]['symbol'] = "O";
elements[8]['name'] = "Oxygen";
elements[8]['weight'] = 15.999;

elements[9] = {};
elements[9]['number'] = 9;
elements[9]['symbol'] = "F";
elements[9]['name'] = "Flourine";
elements[9]['weight'] = 18.998;

elements[10] = {};
elements[10]['number'] = 10;
elements[10]['symbol'] = "Ne";
elements[10]['name'] = "Neon";
elements[10]['weight'] = 20.179;

elements[11] = {};
elements[11]['number'] = 11;
elements[11]['symbol'] = "Na";
elements[11]['name'] = "Sodium";
elements[11]['weight'] = 22.989;

elements[12] = {};
elements[12]['number'] = 12;
elements[12]['symbol'] = "Mg";
elements[12]['name'] = "Magnesium";
elements[12]['weight'] = 24.305;

elements[13] = {};
elements[13]['number'] = 13;
elements[13]['symbol'] = "Al";
elements[13]['name'] = "Aluminium";
elements[13]['weight'] = 26.981;

elements[14] = {};
elements[14]['number'] = 14;
elements[14]['symbol'] = "Si";
elements[14]['name'] = "Silicon";
elements[14]['weight'] = 28.085;

elements[15] = {};
elements[15]['number'] = 15;
elements[15]['symbol'] = "P";
elements[15]['name'] = "Phosphorus";
elements[15]['weight'] = 30.973;

elements[16] = {};
elements[16]['number'] = 16;
elements[16]['symbol'] = "S";
elements[16]['name'] = "Sulfur";
elements[16]['weight'] = 32.065;

elements[17] = {};
elements[17]['number'] = 17;
elements[17]['symbol'] = "Cl";
elements[17]['name'] = "Chlorine";
elements[17]['weight'] = 35.453;

elements[18] = {};
elements[18]['number'] = 18;
elements[18]['symbol'] = "Ar";
elements[18]['name'] = "Argon";
elements[18]['weight'] = 39.948;

elements[19] = {};
elements[19]['number'] = 19;
elements[19]['symbol'] = "K";
elements[19]['name'] = "Potassium";
elements[19]['weight'] = 39.098;

elements[20] = {};
elements[20]['number'] = 20;
elements[20]['symbol'] = "Ca";
elements[20]['name'] = "Calcium";
elements[20]['weight'] = 40.078;

elements[21] = {};
elements[21]['number'] = 21;
elements[21]['symbol'] = "Sc";
elements[21]['name'] = "Scandium";
elements[21]['weight'] = 44.955;

elements[22] = {};
elements[22]['number'] = 22;
elements[22]['symbol'] = "Ti";
elements[22]['name'] = "Titanium";
elements[22]['weight'] = 47.867;

elements[23] = {};
elements[23]['number'] = 23;
elements[23]['symbol'] = "V";
elements[23]['name'] = "Vanadium";
elements[23]['weight'] = 50.941;

elements[24] = {};
elements[24]['number'] = 24;
elements[24]['symbol'] = "Cr";
elements[24]['name'] = "Chromium";
elements[24]['weight'] = 51.996;

elements[25] = {};
elements[25]['number'] = 25;
elements[25]['symbol'] = "Mn";
elements[25]['name'] = "Manganese";
elements[25]['weight'] = 54.938;

elements[26] = {};
elements[26]['number'] = 26;
elements[26]['symbol'] = "Fe";
elements[26]['name'] = "Iron";
elements[26]['weight'] = 55.845;

elements[27] = {};
elements[27]['number'] = 27;
elements[27]['symbol'] = "Co";
elements[27]['name'] = "Cobalt";
elements[27]['weight'] = 58.933;

elements[28] = {};
elements[28]['number'] = 28;
elements[28]['symbol'] = "Ni";
elements[28]['name'] = "Nickel";
elements[28]['weight'] = 58.693;

elements[29] = {};
elements[29]['number'] = 29;
elements[29]['symbol'] = "Cu";
elements[29]['name'] = "Copper";
elements[29]['weight'] = 63.546;

elements[30] = {};
elements[30]['number'] = 30;
elements[30]['symbol'] = "Zn";
elements[30]['name'] = "Zinc";
elements[30]['weight'] = 65.38;

elements[31] = {};
elements[31]['number'] = 31;
elements[31]['symbol'] = "Ga";
elements[31]['name'] = "Gallium";
elements[31]['weight'] = 69.723;

elements[32] = {};
elements[32]['number'] = 32;
elements[32]['symbol'] = "Ge";
elements[32]['name'] = "Germanium";
elements[32]['weight'] = 72.64;

elements[33] = {};
elements[33]['number'] = 33;
elements[33]['symbol'] = "As";
elements[33]['name'] = "Arsenic";
elements[33]['weight'] = 74.921;

elements[34] = {};
elements[34]['number'] = 34;
elements[34]['symbol'] = "Se";
elements[34]['name'] = "Selenium";
elements[34]['weight'] = 78.96;

elements[35] = {};
elements[35]['number'] = 35;
elements[35]['symbol'] = "Br";
elements[35]['name'] = "Bromine";
elements[35]['weight'] = 79.904;

elements[36] = {};
elements[36]['number'] = 36;
elements[36]['symbol'] = "Kr";
elements[36]['name'] = "Krypton";
elements[36]['weight'] = 83.798;

elements[37] = {};
elements[37]['number'] = 37;
elements[37]['symbol'] = "Rb";
elements[37]['name'] = "Rubidium";
elements[37]['weight'] = 85.4678;

elements[38] = {};
elements[38]['number'] = 38;
elements[38]['symbol'] = "Sr";
elements[38]['name'] = "Strontium";
elements[38]['weight'] = 87.62;


elements[39] = {};
elements[39]['number'] = 39;
elements[39]['symbol'] = "Y";
elements[39]['name'] = "Yttrium";
elements[39]['weight'] = 88.905;

elements[40] = {};
elements[40]['number'] = 40;
elements[40]['symbol'] = "Zr";
elements[40]['name'] = "Zirconium";
elements[40]['weight'] = 91.224;

elements[41] = {};
elements[41]['number'] = 41;
elements[41]['symbol'] = "Nb";
elements[41]['name'] = "Niobium";
elements[41]['weight'] = 92.906;

elements[42] = {};
elements[42]['number'] = 42;
elements[42]['symbol'] = "Mo";
elements[42]['name'] = "Molybdenum";
elements[42]['weight'] = 95.96;

elements[43] = {};
elements[43]['number'] = 43;
elements[43]['symbol'] = "Tc";
elements[43]['name'] = "Technetium";
elements[43]['weight'] = 97.907;

elements[44] = {};
elements[44]['number'] = 44;
elements[44]['symbol'] = "Ru";
elements[44]['name'] = "Ruthenium";
elements[44]['weight'] = 91.224;

elements[45] = {};
elements[45]['number'] = 45;
elements[45]['symbol'] = "Rh";
elements[45]['name'] = "Rhodium";
elements[45]['weight'] = 102.90550;

elements[46] = {};
elements[46]['number'] = 46;
elements[46]['symbol'] = "Pd";
elements[46]['name'] = "Palladium";
elements[46]['weight'] = 106.42;

elements[47] = {};
elements[47]['number'] = 47;
elements[47]['symbol'] = "Ag";
elements[47]['name'] = "Silver";
elements[47]['weight'] = 107.868;

elements[48] = {};
elements[48]['number'] = 48;
elements[48]['symbol'] = "Cd";
elements[48]['name'] = "Cadmium";
elements[48]['weight'] = 112.411;

elements[49] = {};
elements[49]['number'] = 49;
elements[49]['symbol'] = "In";
elements[49]['name'] = "Indium";
elements[49]['weight'] = 114.818;

elements[50] = {};
elements[50]['number'] = 50;
elements[50]['symbol'] = "Sn";
elements[50]['name'] = "Tin";
elements[50]['weight'] = 118.710;

elements[51] = {};
elements[51]['number'] = 51;
elements[51]['symbol'] = "Sb";
elements[51]['name'] = "Antimony";
elements[51]['weight'] = 121.760;

elements[52] = {};
elements[52]['number'] = 52;
elements[52]['symbol'] = "Te";
elements[52]['name'] = "Tellurium";
elements[52]['weight'] = 127.60;

elements[53] = {};
elements[53]['number'] = 53;
elements[53]['symbol'] = "I";
elements[53]['name'] = "Iodine";
elements[53]['weight'] = 126.904;

elements[54] = {};
elements[54]['number'] = 54;
elements[54]['symbol'] = "Xe";
elements[54]['name'] = "Xenon";
elements[54]['weight'] = 131.293;

elements[55] = {};
elements[55]['number'] = 55;
elements[55]['symbol'] = "Cs";
elements[55]['name'] = "Caesium";
elements[55]['weight'] = 132.90;

elements[56] = {};
elements[56]['number'] = 56;
elements[56]['symbol'] = "Ba";
elements[56]['name'] = "Barium";
elements[56]['weight'] = 137.327;

elements[57] = {};
elements[57]['number'] = 57;
elements[57]['symbol'] = "La";
elements[57]['name'] = "Lanthanum";
elements[57]['weight'] = 138.90;

elements[58] = {};
elements[58]['number'] = 58;
elements[58]['symbol'] = "Ce";
elements[58]['name'] = "Cerium";
elements[58]['weight'] = 140.116;

elements[59] = {};
elements[59]['number'] = 59;
elements[59]['symbol'] = "Pr";
elements[59]['name'] = "Praseodymium";
elements[59]['weight'] = 140.90;

elements[60] = {};
elements[60]['number'] = 60;
elements[60]['symbol'] = "Nd";
elements[60]['name'] = "Neodymium";
elements[60]['weight'] = 133.242;

elements[61] = {};
elements[61]['number'] = 61;
elements[61]['symbol'] = "Pm";
elements[61]['name'] = "Promehtium";
elements[61]['weight'] = 145;

elements[62] = {};
elements[62]['number'] = 62;
elements[62]['symbol'] = "Sm";
elements[62]['name'] = "Samarium";
elements[62]['weight'] = 150.36;

elements[63] = {};
elements[63]['number'] = 63;
elements[63]['symbol'] = "Eu";
elements[63]['name'] = "Europium";
elements[63]['weight'] = 151.964;

elements[64] = {};
elements[64]['number'] = 64;
elements[64]['symbol'] = "Gd";
elements[64]['name'] = "Gadolinium";
elements[64]['weight'] = 157.25;

elements[65] = {};
elements[65]['number'] = 65;
elements[65]['symbol'] = "Tb";
elements[65]['name'] = "Terbium";
elements[65]['weight'] = 158.92;

elements[66] = {};
elements[66]['number'] = 66;
elements[66]['symbol'] = "Dy";
elements[66]['name'] = "Dysprosium";
elements[66]['weight'] = 162.5;

elements[67] = {};
elements[67]['number'] = 67;
elements[67]['symbol'] = "Ho";
elements[67]['name'] = "Holmium";
elements[67]['weight'] = 164.93;

elements[68] = {};
elements[68]['number'] = 68;
elements[68]['symbol'] = "Er";
elements[68]['name'] = "Erbium";
elements[68]['weight'] = 167.259;

elements[69] = {};
elements[69]['number'] = 69;
elements[69]['symbol'] = "Tm";
elements[69]['name'] = "Thulium";
elements[69]['weight'] = 168.93;

elements[70] = {};
elements[70]['number'] = 70;
elements[70]['symbol'] = "Yb";
elements[70]['name'] = "Ytterbium";
elements[70]['weight'] = 173.054;

elements[71] = {};
elements[71]['number'] = 71;
elements[71]['symbol'] = "Lu";
elements[71]['name'] = "Lutetium";
elements[71]['weight'] = 174.966;

elements[72] = {};
elements[72]['number'] = 72;
elements[72]['symbol'] = "Hf";
elements[72]['name'] = "Halfnium";
elements[72]['weight'] = 178.49;

elements[73] = {};
elements[73]['number'] = 73;
elements[73]['symbol'] = "Ta";
elements[73]['name'] = "Tantaium";
elements[73]['weight'] = 180.94;

elements[74] = {};
elements[74]['number'] = 74;
elements[74]['symbol'] = "W";
elements[74]['name'] = "Tungsten";
elements[74]['weight'] = 183.84;

elements[75] = {};
elements[75]['number'] = 75;
elements[75]['symbol'] = "Re";
elements[75]['name'] = "Rhenium";
elements[75]['weight'] = 186.207;

elements[76] = {};
elements[76]['number'] = 76;
elements[76]['symbol'] = "Os";
elements[76]['name'] = "Osmium";
elements[76]['weight'] = 190.23;

elements[77] = {};
elements[77]['number'] = 77;
elements[77]['symbol'] = "Ir";
elements[77]['name'] = "Iridium";
elements[77]['weight'] = 192.217;

elements[78] = {};
elements[78]['number'] = 78;
elements[78]['symbol'] = "Pt";
elements[78]['name'] = "Platinum";
elements[78]['weight'] = 195.084;

elements[79] = {};
elements[79]['number'] = 79;
elements[79]['symbol'] = "Au";
elements[79]['name'] = "Gold";
elements[79]['weight'] = 196.96;

elements[80] = {};
elements[80]['number'] = 80;
elements[80]['symbol'] = "Hg";
elements[80]['name'] = "Mercury";
elements[80]['weight'] = 200.59;

elements[81] = {};
elements[81]['number'] = 81;
elements[81]['symbol'] = "Tl";
elements[81]['name'] = "Thallium";
elements[81]['weight'] = 204.38;

elements[82] = {};
elements[82]['number'] = 82;
elements[82]['symbol'] = "Pb";
elements[82]['name'] = "Lead";
elements[82]['weight'] = 207.2;

elements[83] = {};
elements[83]['number'] = 83;
elements[83]['symbol'] = "Bi";
elements[83]['name'] = "Bismuth";
elements[83]['weight'] = 208.98;

elements[84] = {};
elements[84]['number'] = 84;
elements[84]['symbol'] = "Po";
elements[84]['name'] = "Polonium";
elements[84]['weight'] = 209;

elements[85] = {};
elements[85]['number'] = 85;
elements[85]['symbol'] = "At";
elements[85]['name'] = "Astatine";
elements[85]['weight'] = 210;

elements[86] = {};
elements[86]['number'] = 86;
elements[86]['symbol'] = "Rn";
elements[86]['name'] = "Radon";
elements[86]['weight'] = 222;

elements[87] = {};
elements[87]['number'] = 87;
elements[87]['symbol'] = "Fr";
elements[87]['name'] = "Francium";
elements[87]['weight'] = 223;

elements[88] = {};
elements[88]['number'] = 88;
elements[88]['symbol'] = "Ra";
elements[88]['name'] = "Radium";
elements[88]['weight'] = 226;

elements[89] = {};
elements[89]['number'] = 89;
elements[89]['symbol'] = "Ac";
elements[89]['name'] = "Actinium";
elements[89]['weight'] = 227;

elements[90] = {};
elements[90]['number'] = 90;
elements[90]['symbol'] = "Th";
elements[90]['name'] = "Thorium";
elements[90]['weight'] = 232.037;

elements[91] = {};
elements[91]['number'] = 91;
elements[91]['symbol'] = "Pa";
elements[91]['name'] = "Proctactinium";
elements[91]['weight'] = 231.035;

elements[92] = {};
elements[92]['number'] = 92;
elements[92]['symbol'] = "U";
elements[92]['name'] = "Uranium";
elements[92]['weight'] = 238.028;

elements[93] = {};
elements[93]['number'] = 93;
elements[93]['symbol'] = "Np";
elements[93]['name'] = "Neptunium";
elements[93]['weight'] = 237;

elements[94] = {};
elements[94]['number'] = 94;
elements[94]['symbol'] = "Pu";
elements[94]['name'] = "Plutonium";
elements[94]['weight'] = 244;

elements[95] = {};
elements[95]['number'] = 95;
elements[95]['symbol'] = "Am";
elements[95]['name'] = "Americium";
elements[95]['weight'] = 243;

elements[96] = {};
elements[96]['number'] = 96;
elements[96]['symbol'] = "Cm";
elements[96]['name'] = "Curium";
elements[96]['weight'] = 247;

elements[97] = {};
elements[97]['number'] = 97;
elements[97]['symbol'] = "Bk";
elements[97]['name'] = "Berkelium";
elements[97]['weight'] = 247;

elements[98] = {};
elements[98]['number'] = 98;
elements[98]['symbol'] = "Cf";
elements[98]['name'] = "Californium";
elements[98]['weight'] = 251;

elements[99] = {};
elements[99]['number'] = 99;
elements[99]['symbol'] = "Es";
elements[99]['name'] = "Einsteiunium";
elements[99]['weight'] = 252;

elements[100] = {};
elements[100]['number'] = 100;
elements[100]['symbol'] = "Fm";
elements[100]['name'] = "Fermium";
elements[100]['weight'] = 257;

elements[101] = {};
elements[101]['number'] = 101;
elements[101]['symbol'] = "Md";
elements[101]['name'] = "Mendelevium";
elements[101]['weight'] = 258;

elements[102] = {};
elements[102]['number'] = 102;
elements[102]['symbol'] = "No";
elements[102]['name'] = "Nobelium";
elements[102]['weight'] = 259;

elements[103] = {};
elements[103]['number'] = 103;
elements[103]['symbol'] = "Lr";
elements[103]['name'] = "Lawrencium";
elements[103]['weight'] = 262;

elements[104] = {};
elements[104]['number'] = 104;
elements[104]['symbol'] = "Rf";
elements[104]['name'] = "Rutherfordium";
elements[104]['weight'] = 267;

elements[105] = {};
elements[105]['number'] = 105;
elements[105]['symbol'] = "Db";
elements[105]['name'] = "Dubnium";
elements[105]['weight'] = 268;

elements[106] = {};
elements[106]['number'] = 106;
elements[106]['symbol'] = "Sg";
elements[106]['name'] = "Seaborgium";
elements[106]['weight'] = 271;

elements[107] = {};
elements[107]['number'] = 107;
elements[107]['symbol'] = "Bh";
elements[107]['name'] = "Bohrium";
elements[107]['weight'] = 272;

elements[108] = {};
elements[108]['number'] = 108;
elements[108]['symbol'] = "Hs";
elements[108]['name'] = "Hassium";
elements[108]['weight'] = 270;

elements[109] = {};
elements[109]['number'] = 109;
elements[109]['symbol'] = "Mt";
elements[109]['name'] = "Meitnerium";
elements[109]['weight'] = 276;

elements[110] = {};
elements[110]['number'] = 110;
elements[110]['symbol'] = "Ds";
elements[110]['name'] = "Darmstadtium";
elements[110]['weight'] = 281;

elements[111] = {};
elements[111]['number'] = 111;
elements[111]['symbol'] = "Rg";
elements[111]['name'] = "Roentgenium";
elements[111]['weight'] = 280;

elements[112] = {};
elements[112]['number'] = 112;
elements[112]['symbol'] = "Cn";
elements[112]['name'] = "Copernicium";
elements[112]['weight'] = 285;

elements[113] = {};
elements[113]['number'] = 113;
elements[113]['symbol'] = "Uut";
elements[113]['name'] = "Ununtrium";
elements[113]['weight'] = 284;

elements[114] = {};
elements[114]['number'] = 114;
elements[114]['symbol'] = "Fl";
elements[114]['name'] = "Flerovium";
elements[114]['weight'] = 289;

elements[115] = {};
elements[115]['number'] = 115;
elements[115]['symbol'] = "Uup";
elements[115]['name'] = "Ununpentium";
elements[115]['weight'] = 288;

elements[116] = {};
elements[116]['number'] = 116;
elements[116]['symbol'] = "Lv";
elements[116]['name'] = "Livermonium";
elements[116]['weight'] = 293;

elements[117] = {};
elements[117]['number'] = 117;
elements[117]['symbol'] = "Uus";
elements[117]['name'] = "Ununspetium";
elements[117]['weight'] = 294;

elements[118] = {};
elements[118]['number'] = 118;
elements[118]['symbol'] = "Uuo";
elements[118]['name'] = "Ununoctium";
elements[118]['weight'] = 294;














jQuery('.element').each(function(){
  var number = $(this).attr('data-number');

  $(this).find('.atomicNumber').html(elements[number].number)
  $(this).find('.atomicWeight').html(elements[number].weight)
  $(this).find('.atomicName').html(elements[number].name)
  $(this).find('.atomicSymbol').html(elements[number].symbol)
});


//onclick for info areas in popup
$('.dropDownHeader').click(function(){
    $(this).find('.arrow').toggleClass('hiddenArrow');
    $(this).next('.infoWrapper').toggleClass('infoOpen')
});
